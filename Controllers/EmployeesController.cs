...
namespace NOC_Management_App.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        [HttpPost]
        [Authorize(Roles = "Admin,Manager")] // Разрешаем создавать и Админам, и Менеджерам
        public async Task<IActionResult> Create([FromBody] CreateEmployeeDto request)
        {
            ...
            try
            {
                ...
                var sql = @"
                    INSERT INTO employees (last_name, first_name, patronymic, role, email, password_hash, last_rest_hours) 
                    VALUES (@LastName, @FirstName, @Patronymic, @Role, @Email, @PasswordHash, 48) 
                    RETURNING id";
                ...
                // ИСПРАВЛЕННЫЙ АУДИТ С БЕЗОПАСНЫМ БЛОКОМ TRY-CATCH И ПРАВИЛЬНЫМ СТОЛБЦОМ user_name
                try
                {
                ...
                {
                    Console.WriteLine($"Ошибка записи в аудит (игнорируем для продолжения работы): {logEx.Message}");
                }
        ...
        public async Task<IActionResult> Update(Guid id, [FromBody] Employee emp)
        {
            ...
            try
            {
                ...
                var sql = "UPDATE employees SET last_name = @LastName, first_name = @FirstName, patronymic = @Patronymic, role = @Role, email = @Email WHERE id = @id";
                await conn.ExecuteAsync(sql, new { LastName = lastName, FirstName = firstName, Patronymic = patronymic, emp.Role, emp.Email, id });
