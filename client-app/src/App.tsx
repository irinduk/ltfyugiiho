...
export default function App() {
    ...
    const handleAssignShift = async (employeeId: string, date: string, type: 'Day' | 'Night' | 'Custom', workAreaId: string, startTime?: string, endTime?: string, isOvertime = false): Promise<boolean> => {
        ...
        // Для корректного расчета часов и проверок ТК РФ на API всегда отправляем startTime/endTime.
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'settings': return <SettingsView theme={theme} setTheme={setTheme} defaultShiftHours={defaultShiftHours} setDefaultShiftHours={setDefaultShiftHours} role={currentRole} passwordChangeDueAt={currentUser?.passwordChangeDueAt} onChangePassword={handleChangePassword} />;
