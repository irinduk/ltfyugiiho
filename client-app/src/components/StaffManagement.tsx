...
import { Employee, Role, Grade } from '../types';

interface StaffManagementProps {
    employees: Employee[];
    allClearances: string[];
    grades?: Grade[]; // Список грейдов (желательно прокинуть из App.tsx)
    onAdd: (data: any) => void;
    onEdit: (id: string, name: string, email: string, role: Role, clearances: string[], gradeId: number | null) => void;
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowAdd(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-500 shadow-lg shadow-blue-900/20 font-bold transition-colors">
                    Добавить сотрудника
                </button>
            </div>
                ...
                <table className="w-full text-left">
                    <thead className={`border-b ${theme === 'dark' ? 'bg-slate-800/50 border-slate-800' : 'bg-slate-200/50 border-slate-200'}`}>
                        <tr>
                            ...
                            <th className={`px-6 py-4 text-xs font-bold uppercase tracking-wider ${theme === 'dark' ? 'text-slate-500' : 'text-slate-900'}`}>Грейд</th>
                    ...
                    <tbody className={`divide-y ${theme === 'dark' ? 'divide-slate-800' : 'divide-slate-200'}`}>
                        {employees.map(e => (
                            <tr key={e.id} className={`transition-colors ${theme === 'dark' ? 'hover:bg-slate-800/30' : 'bg-white hover:bg-slate-50'}`}>
                                ...
                                <td className="px-6 py-4">
                                    {e.gradeId && grades.length > 0 ? (
                                        <span className={`text-xs px-2 py-1 rounded-md border font-bold bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20`}>
                                            {grades.find(g => g.id === e.gradeId)?.name || 'Неизвестно'}
                                        </span>
                                    ) : (
                            ...
                            {editingEmployee ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className={`block text-xs font-bold uppercase mb-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-700'}`}>Грейд</label>
                                            <select id="edit-emp-grade" defaultValue={editingEmployee.gradeId || ''} className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-900 dark:text-white outline-none focus:border-blue-500">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className={`block text-xs font-bold uppercase mb-1 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-700'}`}>Грейд</label>
                                        <select id="add-emp-grade" className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-2 text-sm text-slate-900 dark:text-white outline-none focus:border-blue-500">
                                            <option value="">Не назначен</option>
                                            {grades.map(g => <option key={g.id} value={g.id}>{g.name}</option>)}
                                        </select>
                                    </div>
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        if (editingEmployee) {
                                            ...
                                            const rawGrade = (document.getElementById('edit-emp-grade') as HTMLSelectElement).value;
                                            const gradeId = rawGrade ? parseInt(rawGrade) : null;

                                            if (name && email) {
                                                onEdit(editingEmployee.id, name, email, role, clearances, gradeId);
                                                setShowAdd(false);
                                            ...
                                            const rawGrade = (document.getElementById('add-emp-grade') as HTMLSelectElement).value;
                                            ...
                                            if (email && lastName && firstName && tempPassword) {
                                                ...
                                                const gradeId = rawGrade ? parseInt(rawGrade) : null;
                                                const newEmployeeData = { lastName, firstName, patronymic, email, role, tempPassword, clearances, gradeId };
